 ...
 */
public class TemplateAttributeSourceGenerator {
    ...
    static String generateAttributesForClass(final Class<?> targetClass, boolean separateAttributesClass, String packageOfAttributesClass) {
        ...
        while (currentClass != null && currentClass != Object.class) {
            for (Field field : Arrays.asList(currentClass.getDeclaredFields())) {
                ...
                //add by tank
                //如果是复杂类型就不生成索引，只会生成 int long 类型的索引
                if(field.getType()!=Integer.class&&field.getType()!=int.class&&field.getType()!=Long.class&&field.getType()!=long.class){
                    continue;
                }
     ...
     */
    TemplateAttributeSourceGenerator() {
    }
}
